...
let apiCall = async (api) => {
  try {
    ...
    const apiRoot = 'http://localhost:55555';
...
let drowAllCars=async (str) => {
  let allCars=await apiCall(str);
  console.log(allCars);
  let responsCars=document.getElementById('responsCar');
  let ul=document.createElement("ul");
  for(let i=0; i<allCars.length;i++){
    let li=document.createElement("li");
    li.innerText="vin"+" "+allCars[i].vin+" "+"color"+" "+allCars[i].color;
    ul.appendChild(li);
    ul.appendChild(document.createElement('hr'));
    creatResponsCar(allCars,i);
  }
  responsCars.appendChild(ul);
}

function creatResponsCar(arr,i) {
  let responsCar=document.getElementById('responsCar');
    let responsCarinner=
    "<div class='car'>"+
      "<div class='config'>"+arr[i]['vin']+"</div>"+
      "<div class='config'>"+arr[i]['color']+"</div>"+
      "<div class='config'>"+"i="+i+"</div>"
    "</div>";
  responsCar.innerHTML+=responsCarinner; 
}

drowAllCars('/cars');
// drowAllCars('/search?vin=111&color=red');
apiCall('/add?vin=1111&color=black');